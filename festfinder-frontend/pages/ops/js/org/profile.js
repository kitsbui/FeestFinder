...
export function OrgProfile() {
  ...
  const saveBank = async () => {
    setBusy('bank');
    try { const out = await put('/organizer/bank', { ...bank, accountNo: bank.accountNo.replace(/\s/g, '') }); toast(tx(out.message)); setBank({ bankBin: '', accountNo: '', accountName: '' }); setBankForm(false); reload(true); } catch (err) { toast(errorText(err), 'error'); } finally { setBusy(null); }
  };

  return h(Fragment, null,
    h(PageHeader, {
      title: t('Hồ sơ doanh nghiệp', 'Business profile'),
    ...
    h('div', { className: 'op-grid op-grid--main' },
      h('div', null,
        h(Card, { title: t('Thương hiệu', 'Brand'), icon: 'storefront', sub: t('Hiện công khai', 'Public') },
          h('div', { className: 'op-form-grid' },
            ...
            h(Field, { label: t('Giới thiệu (tiếng Việt)', 'About (Vietnamese)'), className: 'is-wide', counter: [f.bioVi.length, 400] }, h(TextArea, { rows: 3, value: f.bioVi, onChange: set('bioVi'), maxLength: 400 })),
        ...
        h(Card, { title: t('Pháp nhân', 'Legal entity'), icon: 'buildings', sub: t('Chỉ dùng cho kiểm duyệt', 'Moderation only') },
          h('div', { className: 'op-form-grid' },
            ...
            h(Field, { label: t('Mã số thuế', 'Tax code'), error: errors.taxCode, hint: tax ? (/^\d{10,14}$/.test(tax) ? t(`✓ ${tax.length} chữ số`, `✓ ${tax.length} digits`) : t(`${tax.length} chữ số — cần 10 đến 14`, `${tax.length} digits — needs 10 to 14`)) : null },
      ...
      h('div', null,
        h(Card, { title: t('Tài khoản nhận tiền', 'Payout account'), icon: 'bank' },
          ...
          owner && data.bank && !bankForm ? h(Button, { size: 'sm', icon: 'pencil-simple', className: 'op-bank-change', onClick: () => setBankForm(true) }, t('Đổi tài khoản', 'Change account')) : null,
          owner && (!data.bank || bankForm) ? h('div', { className: 'op-bank-form' },
            ...
            h(Field, { label: t('Tên chủ tài khoản', 'Account holder'), hint: t('Viết hoa không dấu', 'Capitals, no accents') }, h(Input, { value: bank.accountName, onChange: (v) => setBank((b) => ({ ...b, accountName: v.toUpperCase() })) })),
            h('div', { className: 'op-form-actions' },
              data.bank ? h(Button, { variant: 'quiet', onClick: () => setBankForm(false) }, t('Huỷ', 'Cancel')) : null,
              h(Button, { busy: busy === 'bank', disabled: !bank.bankBin || bank.accountNo.replace(/\s/g, '').length < 6 || bank.accountName.trim().length < 2, onClick: saveBank }, t('Lưu tài khoản', 'Save account')))) : null),
        h(Card, { title: t('Thành viên', 'Team'), icon: 'users-three' },
          ...
          h('p', { className: 'op-hint' }, t('Thêm người: nhắn FeestFinder qua Hộp thư.', 'To add someone, message FeestFinder in the inbox.'))))));
}
