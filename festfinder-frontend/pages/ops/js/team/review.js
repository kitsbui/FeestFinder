...
export function RejectDialog({ open, onClose, ids, titles, onDone }) {
  ...
  h(Field, { label: t('Lý do', 'Reason'), required: true },
  ...
  h(Field, { label: t('Tin nhắn gửi nhà tổ chức', 'Message to the organizer'), required: true, counter: [message.length, 2000], aside: h('div', { className: 'op-seg op-seg--sm' }, ['vi', 'en'].map((l) => h('button', { key: l, type: 'button', className: cx('op-seg-item', lang === l && 'is-on'), onClick: () => setLang(l) }, l.toUpperCase()))) },
  ...
  h(Switch, { checked: appeal && !!reason?.appealAllowed, disabled: !reason?.appealAllowed, onChange: setAppeal, label: t('Cho phép kháng nghị trong 7 ngày', 'Allow an appeal within 7 days') }));
}
...
function Thread({ id, onSent }) {
  ...
  return h('div', { className: 'op-rv-thread' },
    ...
    h(TextArea, { rows: 3, value: text, onChange: setText, placeholder: t('Nhắn nhà tổ chức…', 'Message the organizer…'), maxLength: 2000 }),
    h('div', { className: 'op-rv-thread-bar' }, h('span', { className: 'op-spacer' }), h(Button, { size: 'sm', icon: 'paper-plane-right', busy, disabled: !text.trim(), onClick: send }, t('Gửi', 'Send'))));
}
...
function ReviewPanel({ id, onDecided, onReject }) {
  ...
  return h('div', { className: 'op-rv' },
    ...
    inReview ? h('div', { className: 'op-rv-foot' },
      ...
      h('span', { className: 'op-kbd-hint' }, h('kbd', null, 'J'), h('kbd', null, 'K'), h('kbd', null, 'A'), h('kbd', null, 'R')),
...
export function Review({ rest }) {
  ...
  const rows = useMemo(() => items
    .filter((i) => view === 'all' || (view === 'flagged' ? i.flagged : view === 'new' ? i.organizer.newOrganizer : i.sla.state === view))
  ...
  [items, view, genre, area, risk, flag, q, sort]);
  ...
  const approveMany = async (ids) => {
    if (ids.length > 1 && !(await confirm({ title: t(`Duyệt ${ids.length} tin?`, `Approve ${ids.length} listings?`), body: t('Tất cả lên sóng ngay.', 'They all go live now.'), confirm: t('Duyệt tất cả', 'Approve all') }))) return;
    setBusy(true);
  ...
  const c = {
    all: items.length, breach: items.filter((i) => i.sla.state === 'breach').length, soon: items.filter((i) => i.sla.state === 'soon').length,
    flagged: items.filter((i) => i.flagged).length, new: items.filter((i) => i.organizer.newOrganizer).length,
  };
  const active = [genre, area, risk, flag].filter(Boolean).length;
  ...
  return h(Fragment, null,
    h(PageHeader, {
      ...
      actions: h(Button, { icon: 'arrow-clockwise', onClick: () => reload(), title: t('Làm mới', 'Refresh') }),
    }),
    h('div', { className: 'op-views' },
      [['all', t('Tất cả', 'All')], ['breach', t('Quá hạn', 'Past SLA')], ['soon', t('Sắp quá hạn', 'Due soon')], ['flagged', t('Bị gắn cờ', 'Flagged')], ['new', t('NTC mới', 'New organizer')]].map(([k, label]) =>
        h('button', { key: k, type: 'button', className: cx('op-view', view === k && 'is-on', k === 'breach' && c[k] && 'is-alert'), onClick: () => setView(k) }, label, h('span', { className: 'op-view-n ff-num' }, data ? c[k] : '·')))),
    h('div', { className: 'op-filterbar' },
      ...
      h('div', { className: 'op-filters' },
        ...
        active ? h('button', { type: 'button', className: 'op-link op-reset', onClick: () => setQuery({ genre: '', area: '', risk: '', flag: '' }) }, Icon('arrow-counter-clockwise'), t('Bỏ lọc', 'Clear')) : null),
      ...
      h('div', { className: 'op-queue op-card', ref: listRef },
        ...
        h('div', { className: 'op-qitem-text' },
          ...
          h('div', { className: 'op-flags' },
            ...
            i.threadMessages ? h(Pill, { icon: 'chat-circle-text' }, i.threadMessages) : null)))) : h(Empty, { icon: items.length ? 'funnel' : 'check-circle', title: items.length ? t('Không có tin khớp bộ lọc', 'No listing matches') : t('Hàng chờ sạch', 'The queue is clear') })),
      h('div', { className: 'op-review-panel op-card' }, current ? h(ReviewPanel, { key: current, id: current, onDecided: decided, onReject: (ids, titles) => setReject({ ids, titles }) }) : h(Empty, { icon: 'cursor-click', title: t('Chọn một tin để xem', 'Pick a listing to review'), body: 'J/K · A · R' }))),
